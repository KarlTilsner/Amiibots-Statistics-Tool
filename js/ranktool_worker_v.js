...
async function rank_tool_leaderboard_count() {
    ...
    const ruleset_id = '&ruleset_id=44748ebb-e2f3-4157-90ec-029e26087ad0';      // vanilla ruleset
    queryURL += ruleset_id;
...
self.onmessage = async function(userInput) {
    ...
    async function get_all_ruleset_amiibo() {
        const url = `https://www.amiibots.com/api/amiibo?per_page=${per_page}&matchmaking_status=ACTIVE,STANDBY&ruleset_id=44748ebb-e2f3-4157-90ec-029e26087ad0`; // vanilla ruleset
        const query = await fetch(url);
        const response = await query.json();
        const data = response.data.map(
            function(index) {
