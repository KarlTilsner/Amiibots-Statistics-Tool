...
async function rank_tool_leaderboard_count() {
    // Check how many amiibo to search for on the leaderboard for b5b
    let queryURL = 'https://www.amiibots.com/api/amiibo?per_page=1&matchmaking_status=ACTIVE,STANDBY';
    const ruleset_id = '&ruleset_id=328d8932-456f-4219-9fa4-c4bafdb55776';      // b5b ruleset
    queryURL += ruleset_id;
...
self.onmessage = async function(userInput) {
    ...
    async function get_all_ruleset_amiibo() {
        const url = `https://www.amiibots.com/api/amiibo?per_page=${per_page}&matchmaking_status=ACTIVE,STANDBY&ruleset_id=328d8932-456f-4219-9fa4-c4bafdb55776`; // b5b ruleset
        const query = await fetch(url);
        const response = await query.json();
        const data = response.data.map(
            function(index) {
